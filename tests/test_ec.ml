...
let key_pair_of_hex h = Mirage_crypto_ec.gen_key ~rng:(fun _ -> Hex.to_cstruct h)
...
let key_exchange =
  let test ~name d p ~expected =
    ( name,
      `Quick,
      fun () ->
        Mirage_crypto_ec.key_exchange d p
...
let scalar_mult =
  let test ~n ~scalar ~point ~expected =
    ...
    ( Printf.sprintf "Scalar mult (#%d)" n,
      `Quick,
      fun () ->
        Mirage_crypto_ec.key_exchange scalar point
        |> Format.asprintf "%a" pp_result
        |> Alcotest.check Alcotest.string __LOC__ expected )
  in
  let point =
...
let scalar_validation =
  let test_scalar_validation ~name ~scalar ~expected =
    ...
    ( name,
      `Quick,
      fun () ->
        let _, _ = Mirage_crypto_ec.gen_key ~rng in
        let got = !ncalls in
