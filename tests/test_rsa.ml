...
let rsa_priv_invalid_d _ =
  ...
  in
  let n = Z.(p * q)
  and dp = Z.(d mod (pred p))
  and dq = Z.(d mod (pred q))
  and q' = Z.(invert q p)
  in
  let actual_d = Z.(invert e ((pred p) * (pred q))) in
  match Rsa.priv ~e ~d ~n ~p ~q ~dp ~dq ~q' with
  | Ok priv when priv.d = actual_d -> ()
  | _ -> assert_failure "expected a valid RSA key"
